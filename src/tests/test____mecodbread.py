...
class TestMECODBRead(unittest.TestCase) :
    ...
    def testSelectRecord(self) :
        ...
        self.lastSeqVal = self.util.getLastSequenceID(self.conn, self.tableName, self.colName)
        row = self.reader.selectRecord(self.conn, self.tableName, self.colName, self.lastSeqVal)
        self.assertEqual(row[self.colName], self.lastSeqVal)

    def tearDown(self) :
        # delete the record that was inserted
        if self.lastSeqVal != None :
            self.deleter.deleteRecord(self.conn, self.tableName, self.colName, self.lastSeqVal)
