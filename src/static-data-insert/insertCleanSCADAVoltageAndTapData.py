...
def getColumns():
	...
	if 'WAILEA/GEN/BESS/KVAR' in header:
		columns['batteryKvar'] = getColumnNumber(header, 'WAILEA/GEN/BESS/KVAR')
	if 'WAILEA/GEN/BESS/KW' in header:
		columns['batteryKw'] = getColumnNumber(header, 'WAILEA/GEN/BESS/KW')
	if 'WAILEA/GEN/BESS/SOC' in header:
		columns['batterySoc'] = getColumnNumber(header, 'WAILEA/GEN/BESS/SOC')
	if 'WAILEA/GEN/BESS_PWR_REF/VOLT' in header:
		columns['batteryVolt'] = getColumnNumber(header, 'WAILEA/GEN/BESS_PWR_REF/VOLT')
...
def writeSeparateFiles(reader, columns):
	...
	if batteryDataPresent:
		outputBatteryDataFile = open('batteryOutput.csv', 'wb')
		writerBattery = csv.writer(outputBatteryDataFile)
		newRowBattery = ['timestamp', 'kvar', 'kw', 'soc', 'pwr_ref_volt']
		writerBattery.writerow(newRowBattery)

	# Build the other csv header rows and write them.
	...
	for row in reader:
		...
		try:
			...
			if batteryDataPresent:
				newRowBattery = [timestamp, row[columns['batteryKvar']], 
						row[columns['batteryKw']],
						row[columns['batterySoc']],
						row[columns['batteryVolt']]]
				writerBattery.writerow(newRowBattery)
...
for filename in fileNames:
	...
	writeNullsCaller(trimmedFileName, 'wailea_voltage_a.csv', 
					columns['transformerVltACol'], columns['timestampCol'])
	writeNullsCaller(trimmedFileName, 'wailea_voltage_b.csv', 
					columns['transformerVltBCol'], columns['timestampCol'])
	writeNullsCaller(trimmedFileName, 'wailea_voltage_c.csv', 
	...
	writeNullsCaller(trimmedFileName, 'battery_kvar.csv', 
					columns['batteryKvar'], columns['timestampCol'])
	writeNullsCaller(trimmedFileName, 'battery_kw.csv', 
					columns['batteryKw'], columns['timestampCol'])
	writeNullsCaller(trimmedFileName, 'battery_soc.csv', 
					columns['batterySoc'], columns['timestampCol'])
	writeNullsCaller(trimmedFileName, 'battery_volt.csv', 
					columns['batteryVolt'], columns['timestampCol'])
