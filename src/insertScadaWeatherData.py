...
"""
One-off script for inserting Kihei SCADA temperature and humidity data.
...
tFiles = ['Kihei AirTemp F 2013_07.csv']
hFiles = ['Kihei_Rel_Humid 2013_07.csv']
...
for i in range(1):
    ...
    with open(hFiles[i], 'rb') as csvfile:
        ...
        for row in myReader:
            ...
            sql = """UPDATE "%s" SET (%s) VALUES (%s) WHERE timestamp =
            TIMESTAMP '%s'""" % (
                table, ','.join(hCols), "'" + row[1].strip() + "'", row[0])
